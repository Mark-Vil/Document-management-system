...
// Fetch submissions from the submission_status table without constraints on status
$sql = "
    SELECT ss.id, ss.submission_id, a.research_title, a.abstract, ss.dateofsubmission, ss.status,
    'View' AS view, 'Action' AS action
    FROM submission_status ss
    JOIN archive a ON ss.submission_id = a.id
    WHERE ss.submission_code = ? 
    AND YEAR(ss.dateofsubmission) = ?
    AND ss.status IN ('Locked', 'Accepted', 'Rejected')
";
if ($stmt = $conn->prepare($sql)) {
    $stmt->bind_param("si", $adviser_code, $selected_year);
    $stmt->execute();
