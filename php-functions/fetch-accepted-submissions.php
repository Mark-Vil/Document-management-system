...
if ($selected_year > 0) {
    ...
    WHERE (ss.status = 'Accepted' OR ss.status = 'Locked') AND YEAR(ss.dateofsubmission) = ? AND u.UserID = ?
    ";
    ...
    if ($stmt = $conn->prepare($query)) {
        ...
        echo json_encode(['status' => 'success', 'accepted_count' => $data['accepted_count']]);
    } else {
