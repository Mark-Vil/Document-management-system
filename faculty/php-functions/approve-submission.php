...
if (isset($_POST['submission_id'])) {
    ...
    // Update the status to 'Accepted' and clear the comments
    $sql = "UPDATE submission_status SET status = 'Accepted', date_accepted = NOW(), comments = '' WHERE submission_id = ?";
        ...
        if ($stmt->execute()) {
            ...
            if ($user_stmt = $conn->prepare($user_sql)) {
                ...
                if ($email_stmt = $conn->prepare($email_sql)) {
                    ...
                    $message = "Your research submission titled \"$research_title\" has been accepted by your adviser. Check it now!";
                    ...
                    if ($email_response['status'] === 'success') {
                        echo json_encode(['status' => 'success', 'message' => 'Submission approved successfully. An email has been sent to the user.']);
                    } else {
                        echo json_encode(['status' => 'error', 'message' => "Submission approved, but email could not be sent. {$email_response['message']}"]);
                    }
                } else {
            ...
            echo json_encode(['status' => 'error', 'message' => 'Failed to approve submission.']);
        }
        $stmt->close();
    } else {
