...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($ownerStmt = $conn->prepare($checkOwnerSql)) {
        ...
        if ($ownerID == $currentUserID) {
            echo json_encode(['status' => 'success', 'message' => 'Owner view not logged']);
            exit();
        }
    }

    // Check if the view already exists
    $checkSql = "SELECT COUNT(*) FROM views WHERE UserID = ? AND research_id = ?";
    if ($checkStmt = $conn->prepare($checkSql)) {
        ...
        if ($count > 0) {
            echo json_encode(['status' => 'success', 'message' => 'View already logged']);
            exit();
        }
    } else {
    ...
    $sql = "INSERT INTO views (UserID, research_id, college_code, department_code, duration) VALUES (?, ?, ?, ?, ?)";
    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param("iissi", $userID, $researchID, $collegeCode, $departmentCode, $duration);
        if ($stmt->execute()) {
            echo json_encode(['status' => 'success']);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Failed to log view']);
        }
        $stmt->close();
    } else {
