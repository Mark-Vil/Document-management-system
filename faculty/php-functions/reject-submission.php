...
if (isset($_POST['submission_id']) && isset($_POST['comments'])) {
    ...
    $sql = "UPDATE submission_status SET status = 'Rejected', comments = ? WHERE submission_id = ?";
        ...
        if ($stmt->execute()) {
            ...
            if ($user_stmt = $conn->prepare($user_sql)) {
                ...
                if ($email_stmt = $conn->prepare($email_sql)) {
                    ...
                    $message = "Your research submission titled \"$research_title\" has been rejected by your adviser. If you think this is a mistake, contact your adviser immediately. Comments: $comments";
                    ...
                    if ($email_response['status'] === 'success') {
                        echo json_encode(['status' => 'success', 'message' => 'Submission rejected successfully. An email has been sent to the user.']);
                    } else {
                        echo json_encode(['status' => 'error', 'message' => "Submission rejected, but email could not be sent. {$email_response['message']}"]);
                    }
                } else {
            ...
            echo json_encode(['status' => 'error', 'message' => 'Failed to reject submission.']);
        }
        $stmt->close();
    } else {
